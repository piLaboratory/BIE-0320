...
mais escuras têm maior densidade das palntas e as mais claras menor
densidade.

![](images/distribuicaoespacial_especies.png)

Você percebe padrões nos mapas acima? Se há padrão, queremos uma
explicação! Imagine que você tenha também mapas de medidas em cada
quadrícula de cinco variáveis de solo importantes para as plantas:

![](images/distribuicaoespacial_variaveisambientais.png)

Parece haver relação entre od dois mapas? O objetivo da particão da
variação é buscar estas relações. Ou seja, estabelecer relações entre
padrões espaciais das espécies, destas com outras variáveis do
ambiente. Feito isso, é possível quantificar quato da variação de
