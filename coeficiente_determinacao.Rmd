...
Por que elevar ao quadrado os
desvios à média? Bom, primeiro porque a soma dos desvios é sempre
zero... Mas também porque a soma dos desvios ao quadrado tem várias
propriedades estatísticas úteis, como a aditividade que vamos ver em
seguida. 
...
Lembrando que para ver o valor que vc obteve e armazenou neste objeto,
...
variável pela variação de outra. Se a regressão é bem sucedida,
esperamos que reste bem menos variação sem explicação, que chamamos de
**variação residual** da regressão. Esta variação residual é a soma
dos quadrados dos desvios de cada ponto à linha de regressão.
...
regressão. Os resíduos da regressão são bem menores que os desvios em
relação à média, da figura anterior:

![](images/coef_determ2.png)

Como chegamos a estes valores na figura? Vamos calcular passo a
...
depois de considerarmos um efeito linear de X sobre Y. A soma dos
...
componentes de variação, como veremos no roteiro seguinte.

Como já calculamos $V_{total}$ e $V_{resid}$ obtemos a variação
...
Neste caso dizemos que 91% da variação de Y é explicada por X. Nada
mal. Mas o que você poderia esperar de dados que a gente mesmo criou,
né!
